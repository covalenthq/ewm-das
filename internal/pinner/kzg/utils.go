...
// Bit masks for efficient bit indexing
const (
...
// Bit positions for setting specific bits in a uint8 value
const (
	bit0 = uint8(1 << iota)
	bit1
	bit2
...
// bitIndex finds the index of the first set bit in a uint32 value.
// Special case: zero has a 0 bit index.
// Example outputs: (0,0), (1,0), (2,1), (3,1), (4,2), etc.
func bitIndex(v uint32) uint8 {
	if v == 0 {
		return 0
	}
	var out uint8
	// Uncomment the following block if mask5
	//if v&mask5 != 0 {
	//	v >>= bit5
	//	out |= bit5
	//}
...
// reverseBitsLimited reverses the bits in value up to the length specified.
...
// ReverseBitsLimited is a public function that calls reverseBitsLimited internally.
func ReverseBitsLimited(length uint32, value uint32) uint32 {
	return reverseBitsLimited(length, value)
}

// reverseBits reverses the bits in a uint32 value using a lookup table for performance.
...
// reverseBitOrder reverses the bit order of indices and applies a swap function for each pair.
func reverseBitOrder(length uint32, swap func(i, j uint32)) {
	if length == 0 || length&(length-1) != 0 {
		panic("length must be a positive power of 2")
	}
	unusedBitLen := 32 - bitIndex(length)
...
// reverseBitOrderFr rearranges elements in a slice of bls.Fr in reverse bit order.
