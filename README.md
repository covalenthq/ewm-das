...
**DAS-Pinner** is a lightweight IPFS service that stores and pins data on the IPFS network using a configurable remote pin backend — either a self-hosted **Kubo** IPFS daemon (via its HTTP RPC) or the managed **Filebase** service (filebase.com). It is designed to work seamlessly with the DAS Light-Client, enabling the retrieval of data from the IPFS network and its verification using the DAS protocol.
